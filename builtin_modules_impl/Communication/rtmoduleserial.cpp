...
void RtModuleSerial::save_frames(bool color, bool depth, bool ir) {
...
//---------------------------------------------------------------------
void RtModuleSerial::start_camera() {
    //запуск камеры
    ...
    if (capture_source == CaptureSourceCamera) {
        ...
        //connect(camera_.data(), &QCamera::stateChanged, this, &RtModuleSerial::on_changed_camera_state);
        //connect(camera_.data(), QOverload<QCamera::Error>::of(&QCamera::error), this, &RtModuleSerial::on_camera_error);
...
RealsenseSettings RtModuleSerial::get_settings() {
    RealsenseSettings s;
...
//---------------------------------------------------------------------
int2 RtModuleSerial::get_res(QString res_string) {    //320_x_240-> 320,240 {
...
//---------------------------------------------------------------------
int RtModuleSerial::get_frame_rate(QString rate_string) {
