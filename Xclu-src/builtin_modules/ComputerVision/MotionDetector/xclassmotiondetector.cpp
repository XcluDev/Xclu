...
void XClassMotionDetector::update() {
    ...
    if (!getobject_input_image()->read().data().cast_copy_to<XRaster>(input0_u8_)) {
        return;
    }
    //no image yet
...
void XClassMotionDetector::bang_on() {
    XModuleUtils::press_button_by_link(get_strings_bang_on());
}

//---------------------------------------------------------------------
void XClassMotionDetector::bang_off() {
    XModuleUtils::press_button_by_link(get_strings_bang_off());
}
