...
void ProjectGui::properties_from_gui() {
    ...
    PROJECT.properties().selected_module = editor_modules->current_index();
}

//---------------------------------------------------------------------
void ProjectGui::properties_to_gui() {
    //выбор модуля
    int i = PROJECT.properties().selected_module;
...
void ProjectGui::rename_module(int i, QString new_name) {
    PROJECT.rename_module(i, new_name);
...
void ProjectGui::duplicate_module(int i) {
    PROJECT.duplicate_module(i);
    editor_modules->inserted_module(i+1, PROJECT.find_module_by_index(i+1)->name());
    //Сигнал, что проект был изменен
    xc_document_modified();
}
