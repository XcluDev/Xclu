...
void EditorModules::update_module_list() {
    moduleListWidget->clear();
    for (int i=0; i<PROJ.modules_count(); i++) {
        inserted_module(i, PROJ.module_by_index(i)->name());
    }
    update_buttons();
}
...
void EditorModules::emit_selection_changed() {  //послать сигнал, что текущий модуль был изменен
    ...
    if (i >= 0 && i < modules_count()) {
        ...
        //auto *interf = PROJ.modules.get_interface(i);
...
void EditorModules::delete_module_click() {
    ...
    int index = current_index();
    if (index < 0 || index > PROJ.modules_count()) return;
    Module *module = PROJ.module_by_index(index);
    if (!module) return;
...
void EditorModules::down_module_click() {
    ...
    int index = current_index();
    if (index >= 0 && index + 1< PROJ.modules_count()) {
